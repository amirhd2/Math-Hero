import { getAssetUrl } from "../utils/assetPaths";
/**
 * Centralized Badge Registry for Math Hero.
 * Defines all 35 data-driven Badges across the 7 core categories:
 * Practice, Accuracy, Improvement, Operations, Streak, Smart Review, and Mastery.
 */

import { Badge } from './gamificationTypes';

export const BADGE_REGISTRY: Badge[] = [
  // ==========================================
  // 1. PRACTICE (تداوم و تمرین)
  // ==========================================
  {
    id: 'first_step',
    imageUrl: getAssetUrl('assets/medals/1.png'),
    name: 'اولین قدم قهرمانی',
    description: 'تکمیل ۳ آزمون ریاضی و شروع سفر ماجراجویی',
    category: 'practice',
    icon: '🌱',
    rarity: 'common',
    xpReward: 30,
    requirement: {
      type: 'quizzes_count',
      target: 3,
      descriptionFa: '۳ آزمون کامل کن',
    },
  },
  {
    id: 'practice_5',
    imageUrl: getAssetUrl('assets/medals/2.png'),
    name: 'هم‌مسیر ریاضی',
    description: 'تکمیل ۱۵ آزمون ریاضی با اراده و پشتکار عالی',
    category: 'practice',
    icon: '🚶‍♂️',
    rarity: 'common',
    xpReward: 50,
    requirement: {
      type: 'quizzes_count',
      target: 15,
      descriptionFa: '۱۵ آزمون کامل کن',
    },
  },
  {
    id: 'practice_15',
    imageUrl: getAssetUrl('assets/medals/3.png'),
    name: 'دوستدار یادگیری',
    description: 'تکمیل ۴۵ آزمون ریاضی؛ تمرین مداوم کلید موفقیت است',
    category: 'practice',
    icon: '📚',
    rarity: 'rare',
    xpReward: 80,
    requirement: {
      type: 'quizzes_count',
      target: 45,
      descriptionFa: '۴۵ آزمون کامل کن',
    },
  },
  {
    id: 'practice_30',
    imageUrl: getAssetUrl('assets/medals/4.png'),
    name: 'قهرمان خستگی‌ناپذیر',
    description: 'تکمیل ۸۰ چالش محاسباتی؛ تو یک قهرمان واقعی هستی',
    category: 'practice',
    icon: '⚡',
    rarity: 'epic',
    xpReward: 150,
    requirement: {
      type: 'quizzes_count',
      target: 80,
      descriptionFa: '۸۰ آزمون کامل کن',
    },
  },
  {
    id: 'practice_50',
    imageUrl: getAssetUrl('assets/medals/5.png'),
    name: 'افسانه تمرین',
    description: '۱۵۰ آزمون کامل شده؛ نام تو در تاریخ قهرمانان ثبت شد',
    category: 'practice',
    icon: '🌟',
    rarity: 'legendary',
    xpReward: 250,
    requirement: {
      type: 'quizzes_count',
      target: 150,
      descriptionFa: '۱۵۰ آزمون کامل کن',
    },
  },

  // ==========================================
  // 2. ACCURACY (دقت و تمرکز)
  // ==========================================
  {
    id: 'first_perfect',
    imageUrl: getAssetUrl('assets/medals/6.png'),
    name: 'ذهن تیزبین',
    description: 'کسب نمره کامل ۱۰۰٪ در ۳ آزمون بدون حتی یک اشتباه',
    category: 'accuracy',
    icon: '🎯',
    rarity: 'common',
    xpReward: 50,
    requirement: {
      type: 'perfect_quizzes',
      target: 3,
      minQuestions: 5,
      descriptionFa: '۳ آزمون بدون خطا کامل کن',
    },
  },
  {
    id: 'perfect_5',
    imageUrl: getAssetUrl('assets/medals/7.png'),
    name: 'پنج ستاره طلایی',
    description: '۱۵ آزمون ۱۰۰٪ بدون خطا؛ تمرکز شما فوق‌العاده است',
    category: 'accuracy',
    icon: '⭐',
    rarity: 'rare',
    xpReward: 100,
    requirement: {
      type: 'perfect_quizzes',
      target: 15,
      minQuestions: 5,
      descriptionFa: '۱۵ آزمون بدون خطا کامل کن',
    },
  },
  {
    id: 'perfect_10',
    imageUrl: getAssetUrl('assets/medals/8.png'),
    name: 'استاد بی‌خطا',
    description: '۲۵ آزمون ۱۰۰٪ بدون خطا؛ استاد بلامنازع دقت',
    category: 'accuracy',
    icon: '💎',
    rarity: 'epic',
    xpReward: 180,
    requirement: {
      type: 'perfect_quizzes',
      target: 25,
      minQuestions: 5,
      descriptionFa: '۲۵ آزمون بدون خطا کامل کن',
    },
  },
  {
    id: 'correct_20',
    imageUrl: getAssetUrl('assets/medals/9.png'),
    name: 'بیست بیست',
    description: 'حل ۵۰ سوال به صورت صحیح',
    category: 'accuracy',
    icon: '✍️',
    rarity: 'common',
    xpReward: 40,
    requirement: {
      type: 'correct_answers',
      target: 50,
      descriptionFa: 'به ۵۰ سوال درست پاسخ بده',
    },
  },
  {
    id: 'correct_100',
    imageUrl: getAssetUrl('assets/medals/10.png'),
    name: 'باشگاه صدتایی‌ها',
    description: 'حل ۳۰۰ سوال ریاضی؛ ماشین حساب زنده!',
    category: 'accuracy',
    icon: '💯',
    rarity: 'rare',
    xpReward: 100,
    requirement: {
      type: 'correct_answers',
      target: 300,
      descriptionFa: 'به ۳۰۰ سوال درست پاسخ بده',
    },
  },
  {
    id: 'correct_300',
    imageUrl: getAssetUrl('assets/medals/11.png'),
    name: 'شکارچی معماها',
    description: 'حل ۸۰۰ سوال صحیح؛ تسلط در سطح المپیاد',
    category: 'accuracy',
    icon: '🏹',
    rarity: 'epic',
    xpReward: 160,
    requirement: {
      type: 'correct_answers',
      target: 800,
      descriptionFa: 'به ۸۰۰ سوال درست پاسخ بده',
    },
  },
  {
    id: 'correct_500',
    imageUrl: getAssetUrl('assets/medals/12.png'),
    name: 'گنجینه دانش',
    description: 'حل ۱۵۰۰ سوال؛ رکوردی دست‌نیافتنی برای قهرمان مطلق',
    category: 'accuracy',
    icon: '🏰',
    rarity: 'legendary',
    xpReward: 250,
    requirement: {
      type: 'correct_answers',
      target: 1500,
      descriptionFa: 'به ۱۵۰۰ سوال درست پاسخ بده',
    },
  },

  // ==========================================
  // 3. IMPROVEMENT (پیشرفت و یادگیری)
  // ==========================================
  {
    id: 'comeback_kid',
    imageUrl: getAssetUrl('assets/medals/13.png'),
    name: 'بازگشت طلایی',
    description: 'پیشرفت چشمگیر و افزایش حداقل ۲۰٪ دقت نسبت به آزمون قبلی',
    category: 'improvement',
    icon: '🚀',
    rarity: 'rare',
    xpReward: 70,
    requirement: {
      type: 'comeback_improvement',
      target: 50,
      descriptionFa: 'دقتت رو حداقل ۲۰٪ نسبت به آزمون قبل ارتقا بده',
    },
  },
  {
    id: 'getting_stronger',
    imageUrl: getAssetUrl('assets/medals/14.png'),
    name: 'روزبه‌روز قوی‌تر',
    description: 'ثبت ۱۰ آزمون پیاپی با نمره ۱۰۰ یا رو‌به‌رشد',
    category: 'improvement',
    icon: '💪',
    rarity: 'rare',
    xpReward: 80,
    requirement: {
      type: 'getting_stronger',
      target: 10,
      descriptionFa: '۱۰ آزمون پشت‌سرهم با نمره عالی ثبت کن',
    },
  },
  {
    id: 'never_give_up',
    imageUrl: getAssetUrl('assets/medals/15.png'),
    name: 'پشتکار قهرمانانه',
    description: 'تمرین دوباره روی سوالات اشتباه و حل موفقیت‌آمیز آن‌ها',
    category: 'improvement',
    icon: '🛡️',
    rarity: 'common',
    xpReward: 50,
    requirement: {
      type: 'resolve_mistakes',
      target: 10,
      descriptionFa: '۳ سوال از گنجینه اشتباهاتت رو با تمرین درست حل کن',
    },
  },

  // ==========================================
  // 4. OPERATIONS (چهار عمل اصلی ریاضی)
  // ==========================================
  // Addition
  {
    id: 'add_starter',
    imageUrl: getAssetUrl('assets/medals/16.png'),
    name: 'شروع با جمع',
    description: 'حل صحیح ۱۵ سوال عمل جمع و ساخت پایه‌های محکم',
    category: 'operations',
    icon: '➕',
    rarity: 'common',
    xpReward: 35,
    requirement: {
      type: 'operation_correct',
      target: 45,
      operation: 'addition',
      descriptionFa: '۱۵ پاسخ درست در عمل جمع ثبت کن',
    },
  },
  {
    id: 'add_pro',
    imageUrl: getAssetUrl('assets/medals/17.png'),
    name: 'حرفه‌ای جمع',
    description: 'حل ۵۰ سوال جمع با دقت بالای ۸۰٪',
    category: 'operations',
    icon: '🟢',
    rarity: 'rare',
    xpReward: 85,
    requirement: {
      type: 'operation_mastery',
      target: 150,
      minAccuracy: 80,
      operation: 'addition',
      descriptionFa: '۵۰ سوال جمع با دقت ۸۰٪ حل کن',
    },
  },
  {
    id: 'add_master',
    imageUrl: getAssetUrl('assets/medals/18.png'),
    name: 'استاد جمع پیشرفته',
    description: 'تسلط کامل و پایدار بر عمل جمع با بیش از ۱۰۰ پاسخ درست',
    category: 'operations',
    icon: '✨',
    rarity: 'epic',
    xpReward: 160,
    requirement: {
      type: 'operation_mastery',
      target: 300,
      minAccuracy: 90,
      operation: 'addition',
      descriptionFa: '۱۰۰ سوال جمع با دقت ۹۰٪ حل کن',
    },
  },

  // Subtraction
  {
    id: 'sub_starter',
    imageUrl: getAssetUrl('assets/medals/19.png'),
    name: 'شروع با تفریق',
    description: 'حل صحیح ۱۵ سوال تفریق و تسلط بر کم کردن اعداد',
    category: 'operations',
    icon: '➖',
    rarity: 'common',
    xpReward: 35,
    requirement: {
      type: 'operation_correct',
      target: 45,
      operation: 'subtraction',
      descriptionFa: '۱۵ پاسخ درست در عمل تفریق ثبت کن',
    },
  },
  {
    id: 'sub_pro',
    imageUrl: getAssetUrl('assets/medals/20.png'),
    name: 'حرفه‌ای تفریق',
    description: 'حل ۵۰ سوال تفریق با دقت بالای ۸۰٪',
    category: 'operations',
    icon: '🔵',
    rarity: 'rare',
    xpReward: 85,
    requirement: {
      type: 'operation_mastery',
      target: 150,
      minAccuracy: 80,
      operation: 'subtraction',
      descriptionFa: '۵۰ سوال تفریق با دقت ۸۰٪ حل کن',
    },
  },
  {
    id: 'sub_master',
    imageUrl: getAssetUrl('assets/medals/21.png'),
    name: 'استاد تفریق پیشرفته',
    description: 'تسلط کامل بر تفریق با بیش از ۱۰۰ پاسخ درست و دقیق',
    category: 'operations',
    icon: '🔷',
    rarity: 'epic',
    xpReward: 160,
    requirement: {
      type: 'operation_mastery',
      target: 300,
      minAccuracy: 90,
      operation: 'subtraction',
      descriptionFa: '۱۰۰ سوال تفریق با دقت ۹۰٪ حل کن',
    },
  },

  // Multiplication
  {
    id: 'mul_starter',
    imageUrl: getAssetUrl('assets/medals/22.png'),
    name: 'شروع با ضرب',
    description: 'حل صحیح ۱۵ سوال جدول ضرب و کشف الگوی شگفت‌انگیز اعداد',
    category: 'operations',
    icon: '✖️',
    rarity: 'common',
    xpReward: 40,
    requirement: {
      type: 'operation_correct',
      target: 45,
      operation: 'multiplication',
      descriptionFa: '۱۵ پاسخ درست در عمل ضرب ثبت کن',
    },
  },
  {
    id: 'mul_pro',
    imageUrl: getAssetUrl('assets/medals/23.png'),
    name: 'حرفه‌ای جدول ضرب',
    description: 'حل ۵۰ سوال ضرب با دقت بالای ۸۰٪ بدون مکث',
    category: 'operations',
    icon: '🟡',
    rarity: 'rare',
    xpReward: 90,
    requirement: {
      type: 'operation_mastery',
      target: 150,
      minAccuracy: 80,
      operation: 'multiplication',
      descriptionFa: '۵۰ سوال ضرب با دقت ۸۰٪ حل کن',
    },
  },
  {
    id: 'mul_master',
    imageUrl: getAssetUrl('assets/medals/24.png'),
    name: 'استاد بی‌رقیب ضرب',
    description: 'تسلط برق‌آسا بر جدول ضرب با ۱۰۰ پاسخ درست و دقیق',
    category: 'operations',
    icon: '👑',
    rarity: 'epic',
    xpReward: 170,
    requirement: {
      type: 'operation_mastery',
      target: 300,
      minAccuracy: 90,
      operation: 'multiplication',
      descriptionFa: '۱۰۰ سوال ضرب با دقت ۹۰٪ حل کن',
    },
  },

  // Division
  {
    id: 'div_starter',
    imageUrl: getAssetUrl('assets/medals/25.png'),
    name: 'شروع با تقسیم',
    description: 'حل صحیح ۱۵ سوال تقسیم عادلانه و بخش‌پذیری',
    category: 'operations',
    icon: '➗',
    rarity: 'common',
    xpReward: 40,
    requirement: {
      type: 'operation_correct',
      target: 45,
      operation: 'division',
      descriptionFa: '۱۵ پاسخ درست در عمل تقسیم ثبت کن',
    },
  },
  {
    id: 'div_pro',
    imageUrl: getAssetUrl('assets/medals/26.png'),
    name: 'حرفه‌ای تقسیم',
    description: 'حل ۵۰ سوال تقسیم با دقت بالای ۸۰٪',
    category: 'operations',
    icon: '🟣',
    rarity: 'rare',
    xpReward: 90,
    requirement: {
      type: 'operation_mastery',
      target: 150,
      minAccuracy: 80,
      operation: 'division',
      descriptionFa: '۵۰ سوال تقسیم با دقت ۸۰٪ حل کن',
    },
  },
  {
    id: 'div_master',
    imageUrl: getAssetUrl('assets/medals/27.png'),
    name: 'استاد تقسیم عادلانه',
    description: 'تسلط بی‌نقص بر تقسیم اعداد با بیش از ۱۰۰ پاسخ درست',
    category: 'operations',
    icon: '🔮',
    rarity: 'epic',
    xpReward: 170,
    requirement: {
      type: 'operation_mastery',
      target: 300,
      minAccuracy: 90,
      operation: 'division',
      descriptionFa: '۱۰۰ سوال تقسیم با دقت ۹۰٪ حل کن',
    },
  },

  // ==========================================
  // 5. STREAK (زنجیره تمرین روزانه)
  // ==========================================
  {
    id: 'streak_3',
    imageUrl: getAssetUrl('assets/medals/28.png'),
    name: '۳ روز با ریاضی',
    description: 'حفظ زنجیره تمرین ریاضی برای ۱۰ روز متوالی',
    category: 'streak',
    icon: '🔥',
    rarity: 'common',
    xpReward: 45,
    requirement: {
      type: 'streak_days',
      target: 10,
      descriptionFa: '۱۰ روز متوالی تمرین کن',
    },
  },
  {
    id: 'streak_7',
    imageUrl: getAssetUrl('assets/medals/29.png'),
    name: 'یک هفته طلایی',
    description: '۲۰ روز تمرین بی‌وقفه؛ نظم رمز پیروزی است',
    category: 'streak',
    icon: '🌟',
    rarity: 'rare',
    xpReward: 90,
    requirement: {
      type: 'streak_days',
      target: 20,
      descriptionFa: '۲۰ روز متوالی تمرین کن',
    },
  },
  {
    id: 'streak_14',
    imageUrl: getAssetUrl('assets/medals/30.png'),
    name: 'دو هفته اراده',
    description: '۴۰ روز مداوم؛ تعهدی تزلزل‌ناپذیر به یادگیری',
    category: 'streak',
    icon: '🏆',
    rarity: 'epic',
    xpReward: 160,
    requirement: {
      type: 'streak_days',
      target: 40,
      descriptionFa: '۴۰ روز متوالی تمرین کن',
    },
  },
  {
    id: 'streak_30',
    imageUrl: getAssetUrl('assets/medals/31.png'),
    name: 'یک ماه قهرمانی',
    description: '۸۰ روز زنجیره تمرین؛ اسطوره استقامت و نظم',
    category: 'streak',
    icon: '👑',
    rarity: 'legendary',
    xpReward: 300,
    requirement: {
      type: 'streak_days',
      target: 80,
      descriptionFa: '۸۰ روز متوالی تمرین کن',
    },
  },

  // ==========================================
  // 6. SMART REVIEW (مرور هوشمند و یادگیری هدفمند)
  // ==========================================
  {
    id: 'smart_first',
    imageUrl: getAssetUrl('assets/medals/32.png'),
    name: 'اولین گام هوشمند',
    description: 'تکمیل اولین جلسه مرور هوشمند و تمرین روی نقاط نیازمند تمرین',
    category: 'smartReview',
    icon: '🧠',
    rarity: 'common',
    xpReward: 40,
    requirement: {
      type: 'smart_review_count',
      target: 3,
      descriptionFa: 'یک جلسه آزمون مرور هوشمند را تمام کن',
    },
  },
  {
    id: 'smart_reviewer',
    imageUrl: getAssetUrl('assets/medals/33.png'),
    name: 'کاشف نقاط عطف',
    description: 'تکمیل ۵ آزمون مرور هوشمند و تقویت هدفمند مهارت‌ها',
    category: 'smartReview',
    icon: '🔍',
    rarity: 'rare',
    xpReward: 85,
    requirement: {
      type: 'smart_review_count',
      target: 15,
      descriptionFa: '۵ آزمون مرور هوشمند را تمام کن',
    },
  },
  {
    id: 'smart_champion',
    imageUrl: getAssetUrl('assets/medals/34.png'),
    name: 'قهرمان یادگیری هدفمند',
    description: 'تکمیل ۱۵ جلسه مرور هوشمند با ارتقای چشمگیر تسلط',
    category: 'smartReview',
    icon: '🎖️',
    rarity: 'epic',
    xpReward: 180,
    requirement: {
      type: 'smart_review_count',
      target: 45,
      descriptionFa: '۱۵ جلسه مرور هوشمند را تمام کن',
    },
  },

  // ==========================================
  // 7. MASTERY (استادی و قهرمان ریاضی)
  // ==========================================
  {
    id: 'all_round_hero',
    imageUrl: getAssetUrl('assets/medals/35.png'),
    name: 'همه‌فن‌حریف',
    description: 'تمرین در هر ۴ عملیات اصلی جمع، تفریق، ضرب و تقسیم',
    category: 'mastery',
    icon: '🌈',
    rarity: 'rare',
    xpReward: 75,
    requirement: {
      type: 'all_operations_tried',
      target: 12,
      descriptionFa: 'در هر ۴ عملیات ریاضی حداقل یک آزمون ثبت کن',
    },
  },
  {
    id: 'speed_accuracy',
    imageUrl: getAssetUrl('assets/medals/36.png'),
    name: 'سریع و تیزبین',
    description: 'تکمیل یک آزمون حداقل ۱۰ سوالی با دقت بالای ۹۰٪',
    category: 'mastery',
    icon: '⚡',
    rarity: 'rare',
    xpReward: 80,
    requirement: {
      type: 'fast_accurate_quiz',
      target: 25,
      minAccuracy: 90,
      descriptionFa: 'یک آزمون ۱۰ سوالی با دقت ۹۰٪ یا بیشتر کامل کن',
    },
  },
  {
    id: 'level_5_milestone',
    imageUrl: getAssetUrl('assets/medals/37.png'),
    name: 'ستاره سطح ۵',
    description: 'رسیدن به سطح ۵ قهرمانی و ارتقای چشمگیر مهارت‌ها',
    category: 'mastery',
    icon: '🥇',
    rarity: 'rare',
    xpReward: 90,
    requirement: {
      type: 'level_reached',
      target: 15,
      descriptionFa: 'به سطح ۵ قهرمانی برس',
    },
  },
  {
    id: 'level_10_milestone',
    imageUrl: getAssetUrl('assets/medals/38.png'),
    name: 'پیشتاز سطح ۱۰',
    description: 'رسیدن به سطح ۱۰ قهرمانی با تجربه فراوان در حل مسئله',
    category: 'mastery',
    icon: '🎖️',
    rarity: 'epic',
    xpReward: 180,
    requirement: {
      type: 'level_reached',
      target: 25,
      descriptionFa: 'به سطح ۱۰ قهرمانی برس',
    },
  },
  // Educational Tier Mastery Badges
  {
    id: 'add_tier1_master',
    imageUrl: getAssetUrl('assets/medals/39.png'),
    name: 'تسلط بر جمع پایه',
    description: 'اثبات علمی تسلط کامل بر جمع یک‌رقمی در یادگیری تطبیقی',
    category: 'mastery',
    icon: '➕',
    rarity: 'common',
    xpReward: 60,
    requirement: {
      type: 'tier_mastered',
      target: 3,
      operation: 'addition',
      tier: 1,
      descriptionFa: 'مرحله ۱ جمع را در سیستم آموزشی تطبیقی به تسلط کامل برسان',
    },
  },
  {
    id: 'sub_tier1_master',
    imageUrl: getAssetUrl('assets/medals/40.png'),
    name: 'تسلط بر تفریق پایه',
    description: 'اثبات علمی تسلط کامل بر تفریق یک‌رقمی در یادگیری تطبیقی',
    category: 'mastery',
    icon: '➖',
    rarity: 'common',
    xpReward: 60,
    requirement: {
      type: 'tier_mastered',
      target: 3,
      operation: 'subtraction',
      tier: 1,
      descriptionFa: 'مرحله ۱ تفریق را در سیستم آموزشی تطبیقی به تسلط کامل برسان',
    },
  },
  {
    id: 'mul_tier1_master',
    imageUrl: getAssetUrl('assets/medals/41.png'),
    name: 'استاد جدول ضرب پایه',
    description: 'اثبات علمی تسلط بر جدول ضرب‌های ۱ تا ۵ در یادگیری تطبیقی',
    category: 'mastery',
    icon: '✖️',
    rarity: 'rare',
    xpReward: 80,
    requirement: {
      type: 'tier_mastered',
      target: 3,
      operation: 'multiplication',
      tier: 1,
      descriptionFa: 'مرحله ۱ ضرب را در سیستم آموزشی تطبیقی به تسلط برسان',
    },
  },
  {
    id: 'div_tier1_master',
    imageUrl: getAssetUrl('assets/medals/42.png'),
    name: 'استاد تقسیم پایه',
    description: 'اثبات علمی تسلط بر تقسیم‌های دقیق پایه در یادگیری تطبیقی',
    category: 'mastery',
    icon: '➗',
    rarity: 'rare',
    xpReward: 80,
    requirement: {
      type: 'tier_mastered',
      target: 3,
      operation: 'division',
      tier: 1,
      descriptionFa: 'مرحله ۱ تقسیم را در سیستم آموزشی تطبیقی به تسلط برسان',
    },
  },
  {
    id: 'two_digit_hero',
    imageUrl: getAssetUrl('assets/medals/43.png'),
    name: 'قهرمان محاسبات دورقمی',
    description: 'تسلط بر مراحل پیشرفته دو‌رقمی (مرحله ۲ یا ۳) در هر یک از عملیات‌ها',
    category: 'mastery',
    icon: '🚀',
    rarity: 'epic',
    xpReward: 120,
    requirement: {
      type: 'tier_mastered',
      target: 5,
      tier: 2,
      descriptionFa: 'حداقل یک مرحله دورقمی یا پیشرفته را به تسلط کامل برسان',
    },
  },
  {
    id: 'quad_master',
    imageUrl: getAssetUrl('assets/medals/44.png'),
    name: 'قهرمان چهارگانه',
    description: 'یادگیری متعادل و همه‌جانبه: تسلط بر حداقل یک مرحله در هر ۴ عملیات ریاضی',
    category: 'mastery',
    icon: '⭐',
    rarity: 'legendary',
    xpReward: 250,
    requirement: {
      type: 'balanced_mastery',
      target: 12,
      descriptionFa: 'در هر ۴ عملیات ریاضی (+، -، ×، ÷) حداقل ۱ مرحله مهارت را مسلط شو',
    },
  },
  {
    id: 'math_hero_grand',
    imageUrl: getAssetUrl('assets/medals/45.png'),
    name: 'قهرمان قهرمانان ریاضی',
    description: 'تاج زرین قهرمانی ریاضی: اثبات تسلط در ۳ عملیات، ۸ مرحله مهارت، سطح ۱۵ و ۱۵ نشان افتخار!',
    category: 'mastery',
    icon: '👑',
    rarity: 'legendary',
    xpReward: 500,
    requirement: {
      type: 'math_hero_grand',
      target: 45,
      descriptionFa: 'احراز تمامی شایستگی‌های علمی و انگیزشی برای کسب تاج الماسین قهرمان ریاضی',
    },
  },
];

export function getBadgeById(id: string): Badge | undefined {
  return BADGE_REGISTRY.find((b) => b.id === id);
}

export function getBadgesByCategory(category: string): Badge[] {
  if (category === 'all') return BADGE_REGISTRY;
  return BADGE_REGISTRY.filter((b) => b.category === category);
}
