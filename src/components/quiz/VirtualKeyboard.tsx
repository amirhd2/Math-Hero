...
export const VirtualKeyboard: React.FC<VirtualKeyboardProps> = ({
  ...
  return (
    <div className="w-full max-w-md lg:max-w-lg mx-auto select-none p-2 sm:p-2.5 bg-slate-100/95 dark:bg-slate-900/95 backdrop-blur-md rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-xl shrink-0 flex flex-col">
      {/* Answer Input Box goes here at the top */}
      {children && (
        <div className="mb-1.5 sm:mb-2 w-full shrink-0">
          {children}
        </div>
      )}

      {/* Main 3-Column Keypad (LTR order so 1 is top-left, 3 is top-right) */}
      <div className="grid grid-cols-3 grid-rows-4 gap-1.5 sm:gap-2 text-center" dir="ltr">
        {/* Row 1: 1, 2, 3 */}
        <button
          type="button"
          onClick={() => handleKeyClick('1')}
          disabled={disabled}
          className="h-10 sm:h-12 md:h-13 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 active:bg-slate-100 dark:active:bg-slate-700 active:scale-95 text-slate-800 dark:text-slate-100 text-xl sm:text-2xl font-black rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-2xs transition-all flex items-center justify-center cursor-pointer disabled:opacity-40"
        >
          {toPersianDigits(1)}
        </button>
        <button
        ...
        <button
          ...
          onClick={onBackspace}
          disabled={disabled}
          className="h-10 sm:h-12 md:h-13 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 active:bg-slate-400 dark:active:bg-slate-600 active:scale-95 text-slate-700 dark:text-slate-200 rounded-2xl border border-slate-300/80 dark:border-slate-700/80 shadow-2xs transition-all flex items-center justify-center cursor-pointer disabled:opacity-40"
          aria-label="پاک کردن"
          title="پاک کردن"
        >
        ...
        <button
          ...
          className={`h-10 sm:h-12 md:h-13 px-1 ${
            isPractice
