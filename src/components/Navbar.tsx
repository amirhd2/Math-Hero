  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between gap-2">
        ...
        <div 
          onClick={() => !isQuizActive && onNavigate(isParentMode ? 'parent_dashboard' : 'profile')} 
          className="flex items-center gap-2.5 cursor-pointer group shrink-0"
          title={isParentMode ? 'میز کار مربی' : 'پروفایل قهرمان'}
        >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isParentMode && (
                <span className="px-2 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-950/80 border border-indigo-300 dark:border-indigo-700 text-indigo-800 dark:text-indigo-300 text-[10px] font-black hidden sm:inline-block">
                  👨‍🏫 والد / مربی
                </span>
              )}
            </div>
        ...
        {/* Left Actions: Mode Switch, PWA Install, Settings */}
          ...
          {!isQuizActive && (
            <>
              {isParentMode ? (
                <div className="flex items-center gap-2">
                  <button
                    id="navbar-parent-dashboard-btn"
                  ...
                  <button
                    id="navbar-exit-to-child-btn"
                    onClick={onExitToChildMode}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 text-xs font-black transition-all cursor-pointer"
                    title="خروج از بخش والدین و بازگشت به محیط کودک"
                  >
                    <span>👦</span>
                    <span className="hidden sm:inline">حالت کودک</span>
                  </button>
                </div>
              ) : (
                <button
                  id="navbar-enter-parent-mode-btn"
                  onClick={onOpenParentGate}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-2xl bg-slate-100 dark:bg-slate-900 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 border border-slate-200 dark:border-slate-800 text-xs font-black transition-all cursor-pointer"
                  title="ورود به بخش والدین و مربیان"
                >
                  <span>👨‍🏫</span>
                  <span className="text-xs">والد / مربی</span>
                </button>
              )}
            </>
          )}
